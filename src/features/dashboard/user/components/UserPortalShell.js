...
function DashboardAccountSidebar({ lexendClassName, onLinkClick = () => {}, showClose, onClose }) {
  ...
  const isSettings = pathname.startsWith("/dashboard/user/profile") || pathname.startsWith("/dashboard/user/my-profile");
  return (
    <>
      <div className={`relative mb-6 border-b border-slate-800 px-5 pb-5 ${showClose ? "pr-14" : ""}`}>
        {showClose ? (
          <button
            type="button"
...
export function UserPortalShell({ children, headerCenter, maxWidthClass = "max-w-6xl" }) {
  ...
  return (
    ...
    >
      <aside
        className={`${lexend.className} fixed left-0 top-0 z-50 hidden h-screen w-[272px] flex-col border-r border-slate-800 bg-slate-900 py-5 antialiased lg:flex`}
        aria-label="Account navigation"
      >
        <DashboardAccountSidebar lexendClassName={lexend.className} />
      </aside>

      <header className="fixed left-0 right-0 top-0 z-40 flex flex-col shadow-sm lg:left-[272px] lg:h-16 lg:w-[calc(100%-272px)] lg:flex-row lg:items-center lg:border-b lg:border-slate-200 lg:bg-white/80 lg:px-6 lg:backdrop-blur-md">
        <div
