...
const navLink =
  "flex items-center gap-3 rounded-lg px-3.5 py-2.5 text-sm font-medium text-slate-400 transition-all duration-200 hover:bg-slate-800/50 hover:text-slate-200";
...
export function TurfAdminShell({ children, headerCenter, maxWidthClass = "max-w-7xl" }) {
  ...
  return (
    ...
    >
      <aside
        className={`${lexend.className} fixed left-0 top-0 z-50 hidden h-screen w-[272px] flex-col border-r border-slate-800 bg-slate-900 p-3.5 antialiased shadow-xl lg:flex`}
        aria-label="Turf admin navigation"
      >
        <TurfAdminSidebar />
      </aside>

      <header className="fixed left-0 right-0 top-0 z-40 flex flex-col shadow-sm lg:left-[272px] lg:h-16 lg:w-[calc(100%-272px)] lg:flex-row lg:items-center lg:border-b lg:border-slate-200 lg:bg-white/80 lg:px-5 lg:backdrop-blur-md">
        <div
