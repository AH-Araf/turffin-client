      ...
      <div className="grid gap-4 lg:grid-cols-[300px_minmax(0,1fr)]">
        ...
        <section className="space-y-4">
          <div className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
            ...
            <div className="flex items-center justify-between gap-3 rounded-lg bg-turf-surface-low px-3 py-2">
              <p className="text-xs font-semibold text-turf-on-surface">{selectedSlots.size} slot{selectedSlots.size === 1 ? "" : "s"} · {dateSummary} · {timeSummary}</p>
                ...
                {dayStrip.map((d) => {
                  ...
                  return (
                    <button
                      ...
                      className={[
                        "flex min-w-16 flex-col items-center rounded-md px-2 py-1.5",
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
              <div className="mb-2 flex items-center justify-between">
                ...
                <span className="text-right text-xs text-turf-on-surface-variant">06:00 AM - 12:00 PM · {BLOCK_RATE_LABELS.morning}</span>
              </div>
              <div className="grid grid-cols-3 gap-2">{model.morning.map((row) => renderSlot(row))}</div>
            </div>

            <div className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
              <div className="mb-2 flex items-center justify-between">
                <h2 className={`${lexendClassName} text-base font-bold text-turf-on-surface`}>Evening</h2>
                <span className="text-right text-xs text-turf-on-surface-variant">12:00 PM - 06:00 PM · {BLOCK_RATE_LABELS.evening}</span>
              </div>
              <div className="grid grid-cols-3 gap-2">{model.evening.map((row) => renderSlot(row))}</div>
            </div>

            <div className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
              <div className="mb-2 flex items-center justify-between">
                <h2 className={`${lexendClassName} text-base font-bold text-turf-on-surface`}>Night</h2>
                <span className="text-right text-xs text-turf-on-surface-variant">06:00 PM - 12:00 AM · {BLOCK_RATE_LABELS.night}</span>
              </div>
              <div className="grid grid-cols-3 gap-2">{model.night.map((row) => renderSlot(row))}</div>
            </div>

            <div className="rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
              <div className="mb-2 flex items-center justify-between">
                <h2 className={`${lexendClassName} text-base font-bold text-turf-on-surface`}>Late night</h2>
                <span className="text-right text-xs text-turf-on-surface-variant">12:00 AM - 06:00 AM · {BLOCK_RATE_LABELS.lateNight}</span>
              </div>
              <div className="grid grid-cols-3 gap-2">{model.lateNight.map((row) => renderSlot(row))}</div>
            </div>
          </div>
        </section>
      </div>
